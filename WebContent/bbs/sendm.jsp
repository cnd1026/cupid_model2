...
<div style="width:100%; height:50px; min-width:1200px; float:left; background-color:#929ADD;">
	<div style="margin-top:10px; margin-left:20px;">
		<font size=4 color=white>보낸메세지</font>
	</div>
</div>
...
<c:choose>
	<c:when test="${count=='0'}">
		<div style="width:60%; margin:0 auto; overflow:hidden; margin-top:20px; margin-bottom:20px; height:300px;  box-shadow:3px 3px 8px 1px gray; display:table; padding-top:170px; text-align:center">보낸메세지없음</div>
	</c:when>
	<c:otherwise>
		<c:forEach var="m" items="${m }">
			<form name="mdel" onsubmit="return send()" action="SendMessageDelete.do?sendemail=${m.sendemail }&&receiveemail=${m.receiveemail}" method="post">
			<div style="width:60%; min-width:900px; height:110px; margin:auto; border-bottom:1px solid; margin-top:20px;">
				<div onclick="location.href='Profile.do?email=${m.receiveemail}'"
				...
				<div style="float:left; width:80%; height:100px">
					...
					<div onclick="location.href='MessageView.do?email=${m.receiveemail}'" class="etc" style="float:left; width:100%; height:50%; padding-left:10px;">
						${m.comment }
					</div>
				</div>
	...
	<div style="float:left; width:10%; height:0px; padding-bottom:8%; color:#565656; margin-left:4%; margin-right:1%; cursor:pointer;">
		<c:if test="${block > 1 }">
			...
			<a href="MessageSendView.do?email=${sessionScope.email}&pageNum=${my_page }">
				<b>&#60;&#60;</b> 처음페이지
			</a>
		</c:if>
	</div>
	...
	<div style="float:left; width:30%; height:0px; padding-bottom:8%; text-align:center;">
		<c:forEach var="i" begin="${first_page + 1 }" end="${last_page }">
			...
			<c:if test="${pageNum != i }">
				<a href="MessageSendView.do?email=${sessionScope.email}&pageNum=${i }" style="font-size:25px; color:#777;">${i }</a>&nbsp;
			</c:if>
		</c:forEach>
	</div>
	...
	<div style="float:left; width:10%; height:0px; padding-bottom:8%; color:#565656; cursor:pointer;">
		<c:if test="${block < total_block }">
			...
			<a href="MessageSendView.do?email=${sessionScope.email}&pageNum=${my_page2 }">
				다음페이지로 <b>&#62;</b>
			</a>
		</c:if>
	</div>
