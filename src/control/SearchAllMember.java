...
@WebServlet("/SearchAllMember.do")
public class SearchAllMember extends HttpServlet {
	private static final long serialVersionUID = 1L;
   
    public SearchAllMember() {
        super();
    }

	protected void doGet(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
		doSearchAllMember(request, response);
	}
	...
	protected void doSearchAllMember(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
		...
		int searchPlatinumCount = dco.searchPlatinumCount(land, gender, firstAgeYear, lastAgeYear);
		...
		// 총 이성 회원 수
		count = dao.searchGetAllMember(land, gender, firstAgeYear, lastAgeYear, searchlevel);
				
		// 입력된 수만큼 회원들 불러오기
		Vector<MemberBean> vs = dpo.searchGetMember(land, gender, startRow, endRow, nowyear, firstAgeYear, lastAgeYear, searchlevel);
		...
		RequestDispatcher dis = request.getRequestDispatcher("/mannam/searchAllMember.jsp?nowserver="+nowserver);
